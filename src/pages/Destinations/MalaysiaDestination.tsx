...
export default function MalaysiaDestination() {
  const [selectedItinerary, setSelectedItinerary] = useState<MalaysiaItinerary | null>(null);
  ...
  return (
    <div className="w-full bg-white text-slate-800 font-sans">
              ...
              >
                ...
                <div className="px-5 sm:px-6 pb-6 pt-0">
                  <button
                    ...
                    className="px-4.5 py-2 text-white text-xs font-semibold rounded-full transition-all duration-300 shadow-sm hover:shadow-md hover:scale-[1.02] flex items-center gap-1.5 cursor-pointer group/btn"
                    style={{
                      background: 'linear-gradient(90deg, #82E58E 0%, #38BDF8 50%, #B872F2 100%)',
      ...
      {/* ─────────────────────────────────────────────────────────────
          MODAL: FULL ITINERARY DETAILS DRAWER / MODAL
      ───────────────────────────────────────────────────────────── */}
      {selectedItinerary && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100 p-6 sm:p-8 relative">
            ...
            {/* Header */}
            <div className="mb-6">
              <span className="inline-block px-3 py-1 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-full text-xs font-bold mb-2">
            ...
            <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700 font-medium">
                {selectedItinerary.highlights.map((hl, i) => (
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {selectedItinerary.days.map((d, i) => (
                  <div key={i} className="flex items-start gap-3 border-l-2 border-emerald-500/40 pl-3">
                    <div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 pt-4 border-t border-gray-100">
              ...
              <a
                href={whatsappInquiryUrl(selectedItinerary.title)}
                target="_blank"
                rel="noopener noreferrer"
                className="px-6 py-2.5 rounded-full text-xs font-bold text-white shadow-md hover:shadow-lg hover:scale-[1.02] transition-all duration-300 flex items-center gap-2 cursor-pointer"
                style={{
                  background: 'linear-gradient(90deg, #82E58E 0%, #38BDF8 50%, #B872F2 100%)',
